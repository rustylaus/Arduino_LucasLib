...
//  Constructor
LD_STRM_HSER::LD_STRM_HSER(byte theUnit, byte theDeviceNumber)
//    : xBee(theRxPort, theTxPort)
{
    setCurrFunction(89);
    deviceBegin(theUnit, theDeviceNumber, false);
    setEnabled(true);
}

//  Destructor
LD_STRM_HSER::~LD_STRM_HSER()
{
    setCurrFunction(0);
    /* nothing to do */
}

void LD_STRM_HSER::streamBegin(long thePortSpeed)
{
    setCurrFunction(88);
    printBegin(thePortSpeed);
    setActive(true);
}

byte LD_STRM_HSER::inputLen()
{
    return(myBuffLen);
}

char LD_STRM_HSER::tokenChar(byte theIX)
{
    return(myToken[theIX]);
}

int LD_STRM_HSER::tokenInt()
{
    setCurrFunction(87);
    //print("Tkn=");
...
String LD_STRM_HSER::tokenString()
{   
    setCurrFunction(86);
    String s = StrNull;
    s.concat(myToken);
...
byte LD_STRM_HSER::tokenLen()
{
    return(myTokenLen);
}

char LD_STRM_HSER::dgType()
{
    return(myCommType);
}

byte LD_STRM_HSER::MaxBuff()
{
    return(MyMaxBuff);
}

byte LD_STRM_HSER::MaxToken()
{
    return(MyMaxToken);
}

byte LD_STRM_HSER::outputInit(char *theBuffer, byte theMaxIX)
...
byte LD_STRM_HSER::outputFill(char *theSource, char *theBuff, byte theBuffOffset, byte theLength)
...
byte LD_STRM_HSER::outputBuild(char *theSource, char *theBuff, byte theLen)
...
byte LD_STRM_HSER::outputSend(char *theBuff, char theType, byte theLength)
...
void LD_STRM_HSER::outputSendChar(char theChar) 
{
    setCurrFunction(81);
    myOC = 0;
    print(theChar);
...
int LD_STRM_HSER::inputAvailable()
{
    setCurrFunction(80);
    myOC = 0;
...
byte LD_STRM_HSER::inputRecv(boolean theDiscard) 
{
    ...
    if (isActive()) 
    {
        ...
        {                                            
            ...
            {
                //delay(100);
                commChar = Serial.read();                                              // Receive a single character from the software serial port
...
byte LD_STRM_HSER::tokenGetLen(int theLength, int theOffset) 
{
...
byte LD_STRM_HSER::tokenGetSep(int theOffset) 
